...
fn init_db(conn: &Connection) {
    let old_schema: Option<String> = conn.query_row(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='device_session'",
        [],
        |r| r.get(0),
    ).ok();
    if let Some(sql) = old_schema {
        if sql.contains("id INTEGER PRIMARY KEY") {
            conn.execute("DROP TABLE device_session", []).ok();
        }
    }
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS device_session (
            role TEXT PRIMARY KEY, uid TEXT, garage_id TEXT, staff_name TEXT, paired_at TEXT
        );",
...
#[tauri::command]
fn save_device_session(db: State<Db>, uid: String, garage_id: String, role: String, staff_name: String) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO device_session (role, uid, garage_id, staff_name, paired_at) VALUES (?1, ?2, ?3, ?4, ?5)
         ON CONFLICT(role) DO UPDATE SET uid=?2, garage_id=?3, staff_name=?4, paired_at=?5",
        params![role, uid, garage_id, staff_name, Utc::now().to_rfc3339()],
    ).map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
fn list_device_sessions(db: State<Db>) -> Result<Vec<DeviceSession>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT uid, garage_id, role, staff_name, paired_at FROM device_session").map_err(|e| e.to_string())?;
    let rows = stmt.query_map([], |r| Ok(DeviceSession {
        uid: r.get(0)?, garage_id: r.get(1)?, role: r.get(2)?,
        staff_name: r.get(3)?, paired_at: r.get(4)?,
    })).map_err(|e| e.to_string())?;
    let mut out = Vec::new();
...
#[tauri::command]
fn clear_device_session(db: State<Db>) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM device_session", []).map_err(|e| e.to_string())?;
    Ok(())
}
...
#[tauri::command]
fn add_client(db: State<Db>, name: String, phone: String, vehicle_plate: String, issue: String) -> Result<Client, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let client = Client {
        id: Uuid::new_v4().to_string(),
        name, phone, vehicle_plate, issue,
        created_at: Utc::now().to_rfc3339(),
        synced: false,
    };
    conn.execute(
        "INSERT INTO clients (id, name, phone, vehicle_plate, issue, created_at, synced) VALUES (?1, ?2, ?3, ?4, ?5, ?6, 0)",
        params![client.id, client.name, client.phone, client.vehicle_plate, client.issue, client.created_at],
    ).map_err(|e| e.to_string())?;
...
fn list_clients(db: State<Db>) -> Result<Vec<Client>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT id, name, phone, vehicle_plate, issue, created_at, synced FROM clients ORDER BY created_at DESC").map_err(|e| e.to_string())?;
    let rows = stmt.query_map([], |r| {
        Ok(Client {
            id: r.get(0)?, name: r.get(1)?, phone: r.get(2)?, vehicle_plate: r.get(3)?,
            issue: r.get(4)?, created_at: r.get(5)?, synced: r.get::<_, i64>(6)? != 0,
        })
    }).map_err(|e| e.to_string())?;
...
#[tauri::command]
fn add_stock_item(db: State<Db>, name: String, qty: i64, unit_price: f64) -> Result<StockItem, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let item = StockItem {
        id: Uuid::new_v4().to_string(), name, qty, unit_price,
        updated_at: Utc::now().to_rfc3339(), synced: false,
    };
    conn.execute(
        "INSERT INTO stock_items (id, name, qty, unit_price, updated_at, synced) VALUES (?1, ?2, ?3, ?4, ?5, 0)",
        params![item.id, item.name, item.qty, item.unit_price, item.updated_at],
    ).map_err(|e| e.to_string())?;
...
fn update_stock_qty(db: State<Db>, id: String, delta: i64) -> Result<StockItem, String> {
    ...
    let item = conn.query_row(
        "SELECT id, name, qty, unit_price, updated_at, synced FROM stock_items WHERE id = ?1",
        params![id],
        |r| Ok(StockItem {
            id: r.get(0)?, name: r.get(1)?, qty: r.get(2)?, unit_price: r.get(3)?,
            updated_at: r.get(4)?, synced: r.get::<_, i64>(5)? != 0,
        }),
    ).map_err(|e| e.to_string())?;
    if item.qty < 0 {
...
fn list_stock(db: State<Db>) -> Result<Vec<StockItem>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT id, name, qty, unit_price, updated_at, synced FROM stock_items ORDER BY name ASC").map_err(|e| e.to_string())?;
    let rows = stmt.query_map([], |r| {
        Ok(StockItem {
            id: r.get(0)?, name: r.get(1)?, qty: r.get(2)?, unit_price: r.get(3)?,
            updated_at: r.get(4)?, synced: r.get::<_, i64>(5)? != 0,
