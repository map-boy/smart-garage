/**
 * The content a brand-new site starts from, and the fallback the renderer
 * uses before Firestore answers.
 *
 * Shared for the same reason the types are: the website draws from it and the
 * admin app resets to it, so one copy keeps a reset from quietly changing what
